...
const computeSupplyIndexUpdate = (
  ...
  if (blockTimestamp.le(VERSION_2_TIMESTAMP)) {
    balance.accumulatedSupplyMorpho = balance.accumulatedSupplyMorpho.plus(accumulatedMorphoV1);
  } else {
...
export const updateSupplyBalanceAndMarket = (
  ...
  if (market.supplyUpdateBlockTimestamp.lt(VERSION_2_TIMESTAMP) && blockTimestamp.ge(VERSION_2_TIMESTAMP)) {
    computeSupplyIndexUpdate(marketAddress, VERSION_2_TIMESTAMP, indexUnits, balance, market);
  }
  computeSupplyIndexUpdate(marketAddress, blockTimestamp, indexUnits, balance, market);
  // accounting
...
const computeBorrowIndexUpdate = (
  ...
  if (blockTimestamp.le(VERSION_2_TIMESTAMP)) {
    balance.accumulatedBorrowMorpho = balance.accumulatedBorrowMorpho.plus(accumulatedMorphoV1);
  } else {
...
export const updateBorrowBalanceAndMarket = (
  ...
  if (market.borrowUpdateBlockTimestamp.lt(VERSION_2_TIMESTAMP) && blockTimestamp.ge(VERSION_2_TIMESTAMP)) {
    computeBorrowIndexUpdate(marketAddress, VERSION_2_TIMESTAMP, indexUnits, balance, market);
  }
  computeBorrowIndexUpdate(marketAddress, blockTimestamp, indexUnits, balance, market);
  // accounting
