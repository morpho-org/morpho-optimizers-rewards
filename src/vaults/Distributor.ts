...
export default class Distributor {
    ...
    constructor(
        vaultAddress: string,
        private eventsFetcher: EventsFetcherInterface,
        private proofsFetcher: ProofsFetcherInterface
    ) {
    ...
    async distributeMorpho(epochToId?: string) {
        ...
        if (!epochsProofs?.length) throw Error(`No MORPHO distributed for the vault ${this.vaultAddress} in epoch ${epochToId}`);
        ...
        for (const epochProofs of epochsProofs) {
                ...
                switch (transaction.type) {
                    case VaultEventType.Deposit: {
                        ...
                        userBalance.morphoAccrued = userBalance.morphoAccrued.add(
                            WadRayMath.wadMul(this._marketIndex.sub(userBalance.index), userBalance.balance).div(WadRayMath.RAY)
                        );
                    ...
                    case VaultEventType.Transfer: {
                        ...
                        userFromBalance.morphoAccrued = userFromBalance.morphoAccrued.add(
                            WadRayMath.wadMul(this._marketIndex.sub(userFromBalance.index), userFromBalance.balance).div(WadRayMath.RAY)
                        );
                        ...
                        userToBalance.morphoAccrued = userToBalance.morphoAccrued.add(
                            WadRayMath.wadMul(this._marketIndex.sub(userToBalance.index), userToBalance.balance).div(WadRayMath.RAY)
                        );
            ...
            Object.values(this._usersConfigs).forEach((userConfig) => {
                ...
                userConfig.morphoAccrued = userConfig.morphoAccrued.add(
                    WadRayMath.wadMul(this._marketIndex.sub(userConfig.index), userConfig.balance).div(WadRayMath.RAY)
                );
